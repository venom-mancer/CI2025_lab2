...
Start from an initial solution and perform standard hill climbing until we reach a local maximum — a point where no neighboring move improves the solution.

Instead of restarting from scratch, we backtrack halfway along the path we used to climb to that peak.
...
From this halfway point, we explore a different branch — meaning we take a different move than before.

We keep a memory (tabu-like list) of moves already tried from that point so we don’t repeat paths we already explored.

We continue this process to discover multiple local peaks, storing each peak in a list of candidate solutions.

After collecting several peaks, we compare them and select the best one as our current best solution.

Once the algorithm has explored all reasonable variations near these peaks, we allow a controlled downhill move or “jump” away from these areas (diversification).
...
We repeat this process — climb, partial backtrack, branch, save peaks, diversify — until convergence or a stopping condition is reached.
